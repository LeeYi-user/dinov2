...
def get_csv_fpath(split):
    ...
    """
    if split == _Split.TRAIN.value.upper():
        return "2022_07_04_whole_image_train_data/whole_images_512_train.csv"
    elif split == _Split.VAL.value.upper():
        return "2022_07_04_whole_image_train_data/whole_images_512_test.csv"
        ...
        for row in reader:
            ...
            if add_sample:
                ...
                if os.path.exists(candidate_path):
                    ...
                    candidate_path = os.path.join(data_path, row["file"].split("/")[-1].split(".")[0] + "_blue.png")
                    # some images on the normalized_data folder have a _blue suffix on their names
                    if os.path.exists(candidate_path):
                        image_paths.append(candidate_path)
                    else:
