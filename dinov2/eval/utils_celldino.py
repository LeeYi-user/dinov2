...
def extract_features_celldino(
    ...
    return extract_features_with_dataloader_celldino(model, data_loader, sample_count, gather_on_cpu, avgpool=avgpool)


@torch.inference_mode()
def extract_features_with_dataloader_celldino(model, data_loader, sample_count, gather_on_cpu=False, avgpool=False):
...
def extract_features_for_dataset_dict(
    ...
    for try_n, dataset in dataset_dict.items():
        features, labels = extract_features_celldino(
